...
from models import db, Product, TicketHeader, TicketLine, ScanLog, Client
from forms import SearchForm, FilterForm, ManualScanForm, ClientSearchForm, ClientForm
...
def clients():
    ...
    if request.args.get('query'):
        search_term = f"%{request.args.get('query')}%"
        query = query.filter(or_(
            Client.Nombre.like(search_term),
            Client.DNI.like(search_term),
            Client.Telefono1.like(search_term),
            Client.Telefono2.like(search_term),
            Client.Telefono3.like(search_term)
        ))
        search_form.query.data = request.args.get('query')
    
    # Execute paginated query
    clients = query.order_by(Client.Nombre).paginate(page=page, per_page=per_page)
    
    return render_template('warehouse/clients.html', 
                          clients=clients,
                          search_form=search_form)

@warehouse_bp.route('/client/<int:client_id>')
@login_required
def client_detail(client_id):
    """Detailed view of a single client"""
    client = Client.query.get_or_404(client_id)
    return render_template('warehouse/client_detail.html', client=client)

@warehouse_bp.route('/client/new', methods=['GET', 'POST'])
@login_required
def client_new():
    """Create a new client"""
    form = ClientForm()
    ...
    if form.validate_on_submit():
        try:
            ...
            # Ottieni il massimo ID cliente esistente e incrementalo
            max_id = db.session.query(func.max(Client.IdCliente)).scalar() or 0
            client.IdCliente = max_id + 1
            
            # Set client fields from form
            client.Nombre = form.nombre.data
            client.DNI = form.dni.data
            client.Direccion = form.direccion.data
            client.CodPostal = form.cod_postal.data
            client.Poblacion = form.poblacion.data
            client.Provincia = form.provincia.data
            client.Pais = form.pais.data
            client.Telefono1 = form.telefono1.data
            client.Telefono2 = form.telefono2.data
            client.Telefono3 = form.telefono3.data
            client.Email = form.email.data
            client.TipoEmailTicket = form.tipo_email_ticket.data
            client.TipoEmailAlbaran = form.tipo_email_albaran.data
            client.TipoEmailFactura = form.tipo_email_factura.data
            client.IdTarifa = form.id_tarifa.data
            client.Ofertas = form.ofertas.data
            client.IdFormaPago = form.id_forma_pago.data
            client.Descuento = form.descuento.data
            client.DtoProntoPago = form.dto_pronto_pago.data
            client.NombreBanco = form.nombre_banco.data
            client.CodigoCuenta = form.codigo_cuenta.data
            client.NumeroVencimientos = form.numero_vencimientos.data
            client.DiasEntreVencimientos = form.dias_entre_vencimientos.data
            client.IdEstado = form.id_estado.data
            client.Observaciones = form.observaciones.data
            client.CodInterno = form.cod_interno.data
            client.EANScanner = form.ean_scanner.data
            client.PuntosFidelidad = form.puntos_fidelidad.data
            client.UsarRecargoEquivalencia = form.usar_recargo_equivalencia.data
            client.TotalPorArticulo = form.total_por_articulo.data
            client.AplicarTarifaEtiqueta = form.aplicar_tarifa_etiqueta.data
            client.FormatoAlbaran = form.formato_albaran.data
            client.FormatoFactura = form.formato_factura.data
            client.ModoFacturacion = form.modo_facturacion.data
            client.TimeStamp = datetime.now()
            client.Usuario = current_user.username
            
            # Handle photo upload if provided
            if form.foto.data:
                # Save photo data
                client.Foto = form.foto.data.read()
            ...
            return redirect(url_for('warehouse.client_detail', client_id=client.IdCliente))
        except Exception as e:
            db.session.rollback()
            flash(f'Errore durante il salvataggio del cliente: {str(e)}', 'danger')
            logger.error(f"Errore salvataggio cliente: {str(e)}")
    
    return render_template('warehouse/client_form.html', 
                          form=form, 
                          title='Nuovo Cliente')

@warehouse_bp.route('/client/<int:client_id>/edit', methods=['GET', 'POST'])
@login_required
def client_edit(client_id):
    ...
    if request.method == 'GET':
        ...
        form.dni.data = client.DNI
        form.direccion.data = client.Direccion
        form.cod_postal.data = client.CodPostal
        form.poblacion.data = client.Poblacion
        form.provincia.data = client.Provincia
        form.pais.data = client.Pais
        form.telefono1.data = client.Telefono1
        form.telefono2.data = client.Telefono2
        form.telefono3.data = client.Telefono3
        form.email.data = client.Email
        form.tipo_email_ticket.data = client.TipoEmailTicket
        form.tipo_email_albaran.data = client.TipoEmailAlbaran
        form.tipo_email_factura.data = client.TipoEmailFactura
        form.id_tarifa.data = client.IdTarifa
        form.ofertas.data = client.Ofertas
        form.id_forma_pago.data = client.IdFormaPago
        form.descuento.data = client.Descuento
        form.dto_pronto_pago.data = client.DtoProntoPago
        form.nombre_banco.data = client.NombreBanco
        form.codigo_cuenta.data = client.CodigoCuenta
        form.numero_vencimientos.data = client.NumeroVencimientos
        form.dias_entre_vencimientos.data = client.DiasEntreVencimientos
        form.id_estado.data = client.IdEstado
        form.observaciones.data = client.Observaciones
        form.cod_interno.data = client.CodInterno
        form.ean_scanner.data = client.EANScanner
        form.puntos_fidelidad.data = client.PuntosFidelidad
        form.usar_recargo_equivalencia.data = client.UsarRecargoEquivalencia
        form.total_por_articulo.data = client.TotalPorArticulo
        form.aplicar_tarifa_etiqueta.data = client.AplicarTarifaEtiqueta
        form.formato_albaran.data = client.FormatoAlbaran
        form.formato_factura.data = client.FormatoFactura
        form.modo_facturacion.data = client.ModoFacturacion
...
@warehouse_bp.route('/client/<int:client_id>/delete', methods=['POST'])
@login_required
def client_delete(client_id):
    ...
    try:
        ...
        db.session.rollback()
        flash(f'Errore durante l\'eliminazione del cliente: {str(e)}', 'danger')
    
    return redirect(url_for('warehouse.clients')) 