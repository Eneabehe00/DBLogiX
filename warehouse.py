...
def index():
    ...
    # Get recent scans by current user (limit to 5)
    recent_scans = ScanLog.query.filter_by(user_id=current_user.id).order_by(
        ScanLog.timestamp.desc()).limit(5).all()
...
def api_scan():
    ...
    if not match:
        return jsonify({
            'success': False, 
            'error': 'Invalid ticket format. Expected: T[number]-P[product]'
        }), 400
