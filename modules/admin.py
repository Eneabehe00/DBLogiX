...
from app.models import db, User, ScanLog, Product, TicketHeader, Company, SystemConfig, ChatMessage, Client, AlbaranCabecera, AlbaranLinea
from app.forms import RegistrationForm, DbConfigForm, CompanyConfigForm, ResetPasswordForm, SystemConfigForm
from sqlalchemy import func, desc
from app.config import (REMOTE_DB_CONFIG, update_db_config, reload_config,
                      get_company_config_from_file, get_system_config_from_file,
                      get_chat_config_from_file, get_clienti_config_from_file,
                      get_ddt_config_from_file, get_fatture_config_from_file,
                      update_company_config, update_system_config,
                      update_chat_config, update_clienti_config,
                      update_ddt_config, update_fatture_config)
import pymysql
...
def general_config():
    ...
    if request.method == 'GET':
        ...
        # Company form - Prima prova dal file di configurazione
        company_config = get_company_config_from_file()
        if company_config and any(company_config.values()):
            # Usa i dati dal file di configurazione
            company_form.nombre_empresa.data = company_config.get('nombre_empresa', '')
            company_form.cif_vat.data = company_config.get('cif_vat', '')
            company_form.telefono.data = company_config.get('telefono', '')
            company_form.direccion.data = company_config.get('direccion', '')
            company_form.cod_postal.data = company_config.get('cod_postal', '')
            company_form.poblacion.data = company_config.get('poblacion', '')
            company_form.provincia.data = company_config.get('provincia', '')
        else:
            # Fallback al database se il file non contiene dati azienda
        ...
        # System form - Prima prova dal file di configurazione
        system_config = get_system_config_from_file()
        chat_config = get_chat_config_from_file()
        clienti_config = get_clienti_config_from_file()
        ddt_config = get_ddt_config_from_file()
        fatture_config = get_fatture_config_from_file()
        
        # Basic system configurations
        system_form.expiry_warning_days.data = system_config.get('expiry_warning_days', SystemConfig.get_config('expiry_warning_days', 7))
        system_form.articles_per_package.data = system_config.get('articles_per_package', SystemConfig.get_config('articles_per_package', 5))
        
        # Email configurations
        system_form.smtp_server.data = system_config.get('smtp_server', SystemConfig.get_config('smtp_server', ''))
        system_form.smtp_port.data = system_config.get('smtp_port', SystemConfig.get_config('smtp_port', 587))
        system_form.smtp_username.data = system_config.get('smtp_username', SystemConfig.get_config('smtp_username', ''))
        system_form.smtp_password.data = system_config.get('smtp_password', SystemConfig.get_config('smtp_password', ''))
        system_form.smtp_use_tls.data = system_config.get('smtp_use_tls', SystemConfig.get_config('smtp_use_tls', True))
        system_form.admin_email.data = system_config.get('admin_email', SystemConfig.get_config('admin_email', ''))
        system_form.enable_email_notifications.data = system_config.get('enable_email_notifications', SystemConfig.get_config('enable_email_notifications', False))
        
        # Backup configurations
        system_form.backup_frequency_hours.data = system_config.get('backup_frequency_hours', SystemConfig.get_config('backup_frequency_hours', 24))
        system_form.backup_retention_days.data = system_config.get('backup_retention_days', SystemConfig.get_config('backup_retention_days', 7))
        system_form.backup_path.data = system_config.get('backup_path', SystemConfig.get_config('backup_path', 'backups'))
        
        # Database timeout configurations
        system_form.db_connect_timeout.data = system_config.get('db_connect_timeout', SystemConfig.get_config('db_connect_timeout', 10))
        system_form.db_read_timeout.data = system_config.get('db_read_timeout', SystemConfig.get_config('db_read_timeout', 30))
        system_form.db_write_timeout.data = system_config.get('db_write_timeout', SystemConfig.get_config('db_write_timeout', 30))
        
        # Localization configurations
        system_form.timezone.data = system_config.get('timezone', SystemConfig.get_config('timezone', 'Europe/Rome'))
        system_form.date_format.data = system_config.get('date_format', SystemConfig.get_config('date_format', '%d/%m/%Y'))
        
        # Alert configurations
        system_form.enable_stock_alerts.data = system_config.get('enable_stock_alerts', SystemConfig.get_config('enable_stock_alerts', True))
        system_form.stock_alert_threshold.data = system_config.get('stock_alert_threshold', SystemConfig.get_config('stock_alert_threshold', 10))
        system_form.expiry_check_frequency_hours.data = system_config.get('expiry_check_frequency_hours', SystemConfig.get_config('expiry_check_frequency_hours', 6))
        
        # Logging configurations
        system_form.log_level.data = system_config.get('log_level', SystemConfig.get_config('log_level', 'INFO'))
        system_form.log_max_size_mb.data = system_config.get('log_max_size_mb', SystemConfig.get_config('log_max_size_mb', 10))
        
        # Session configurations
        system_form.session_timeout_hours.data = system_config.get('session_timeout_hours', SystemConfig.get_config('session_timeout_hours', 2))
        system_form.session_inactivity_minutes.data = system_config.get('session_inactivity_minutes', SystemConfig.get_config('session_inactivity_minutes', 30))
        
        # Chat configurations
        system_form.enable_chat_auto_cleanup.data = chat_config.get('enable_chat_auto_cleanup', SystemConfig.get_config('enable_chat_auto_cleanup', False))
        system_form.chat_cleanup_frequency_days.data = chat_config.get('chat_cleanup_frequency_days', SystemConfig.get_config('chat_cleanup_frequency_days', 7))
        system_form.enable_chat_auto_backup.data = chat_config.get('enable_chat_auto_backup', SystemConfig.get_config('enable_chat_auto_backup', True))
        system_form.chat_backup_retention_days.data = chat_config.get('chat_backup_retention_days', SystemConfig.get_config('chat_backup_retention_days', 30))
        system_form.chat_backup_path.data = chat_config.get('chat_backup_path', SystemConfig.get_config('chat_backup_path', 'Backup/Chat'))
        
        # Clienti backup configurations
        system_form.enable_clienti_auto_backup.data = clienti_config.get('enable_clienti_auto_backup', SystemConfig.get_config('enable_clienti_auto_backup', True))
        system_form.clienti_backup_frequency_days.data = clienti_config.get('clienti_backup_frequency_days', SystemConfig.get_config('clienti_backup_frequency_days', 7))
        system_form.clienti_backup_retention_days.data = clienti_config.get('clienti_backup_retention_days', SystemConfig.get_config('clienti_backup_retention_days', 30))
        system_form.clienti_backup_path.data = clienti_config.get('clienti_backup_path', SystemConfig.get_config('clienti_backup_path', 'Backup/Clienti'))
        
        # DDT backup configurations
        system_form.enable_ddt_auto_backup.data = ddt_config.get('enable_ddt_auto_backup', SystemConfig.get_config('enable_ddt_auto_backup', True))
        system_form.ddt_backup_frequency_days.data = ddt_config.get('ddt_backup_frequency_days', SystemConfig.get_config('ddt_backup_frequency_days', 7))
        system_form.ddt_backup_retention_days.data = ddt_config.get('ddt_backup_retention_days', SystemConfig.get_config('ddt_backup_retention_days', 30))
        system_form.ddt_backup_path.data = ddt_config.get('ddt_backup_path', SystemConfig.get_config('ddt_backup_path', 'Backup/DDT'))
        
        # Fatture backup configurations
        system_form.enable_fatture_auto_backup.data = fatture_config.get('enable_fatture_auto_backup', SystemConfig.get_config('enable_fatture_auto_backup', True))
        system_form.fatture_backup_frequency_days.data = fatture_config.get('fatture_backup_frequency_days', SystemConfig.get_config('fatture_backup_frequency_days', 7))
        system_form.fatture_backup_retention_days.data = fatture_config.get('fatture_backup_retention_days', SystemConfig.get_config('fatture_backup_retention_days', 30))
        system_form.fatture_backup_path.data = fatture_config.get('fatture_backup_path', SystemConfig.get_config('fatture_backup_path', 'Backup/Fatture'))
    ...
    if 'submit_db' in request.form and db_form.validate():
        if 'test_connection' in request.form:
            ...
            try:
                ...
                # Usa la funzione centralizzata per aggiornare la configurazione
                update_db_config(new_config)
                
                flash('Configurazione database aggiornata nel file DBLogix.exe.config!', 'success')
            except Exception as e:
    ...
    if 'submit_company' in request.form and company_form.validate():
        try:
            # Prepara i dati azienda per il file di configurazione
            company_config = {
                'nombre_empresa': company_form.nombre_empresa.data,
                'cif_vat': company_form.cif_vat.data,
                'telefono': company_form.telefono.data,
                'direccion': company_form.direccion.data,
                'cod_postal': company_form.cod_postal.data,
                'poblacion': company_form.poblacion.data,
                'provincia': company_form.provincia.data
            }
            
            # Salva nel file di configurazione esterno
            update_company_config(company_config)
            
            # Mantieni anche la compatibilità con il database per ora
            try:
                company = Company.query.first()
                if not company:
                    company = Company()
                ...
                # Se il salvataggio nel database fallisce, continuiamo comunque
                logger.warning(f"Database company update failed: {db_error}")
            
            flash('Configurazione azienda aggiornata nel file DBLogix.exe.config!', 'success')
        except Exception as e:
            flash(f'Errore nel salvataggio: {str(e)}', 'danger')
    ...
    if 'submit_system' in request.form:
        ...
        if system_form.validate():
            try:
                ...
                # Prepara le configurazioni per il file esterno
                system_config_data = {
                    'expiry_warning_days': system_form.expiry_warning_days.data,
                    'articles_per_package': system_form.articles_per_package.data,
                    'timezone': system_form.timezone.data,
                    'date_format': system_form.date_format.data,
                    'smtp_server': system_form.smtp_server.data,
                    'smtp_port': system_form.smtp_port.data,
                    'smtp_username': system_form.smtp_username.data,
                    'smtp_password': system_form.smtp_password.data,
                    'smtp_use_tls': system_form.smtp_use_tls.data,
                    'admin_email': system_form.admin_email.data,
                    'enable_email_notifications': system_form.enable_email_notifications.data,
                    'backup_frequency_hours': system_form.backup_frequency_hours.data,
                    'backup_retention_days': system_form.backup_retention_days.data,
                    'backup_path': system_form.backup_path.data,
                    'db_connect_timeout': system_form.db_connect_timeout.data,
                    'db_read_timeout': system_form.db_read_timeout.data,
                    'db_write_timeout': system_form.db_write_timeout.data,
                    'enable_stock_alerts': system_form.enable_stock_alerts.data,
                    'stock_alert_threshold': system_form.stock_alert_threshold.data,
                    'expiry_check_frequency_hours': system_form.expiry_check_frequency_hours.data,
                    'log_level': system_form.log_level.data,
                    'log_max_size_mb': system_form.log_max_size_mb.data,
                    'session_timeout_hours': system_form.session_timeout_hours.data,
                    'session_inactivity_minutes': system_form.session_inactivity_minutes.data
                }
                
                chat_config_data = {
                    'enable_chat_auto_cleanup': system_form.enable_chat_auto_cleanup.data,
                    'chat_cleanup_frequency_days': system_form.chat_cleanup_frequency_days.data,
                    'enable_chat_auto_backup': system_form.enable_chat_auto_backup.data,
                    'chat_backup_retention_days': system_form.chat_backup_retention_days.data,
                    'chat_backup_path': system_form.chat_backup_path.data
                }
                
                clienti_config_data = {
                    'enable_clienti_auto_backup': system_form.enable_clienti_auto_backup.data,
                    'clienti_backup_frequency_days': system_form.clienti_backup_frequency_days.data,
                    'clienti_backup_retention_days': system_form.clienti_backup_retention_days.data,
                    'clienti_backup_path': system_form.clienti_backup_path.data
                }
                
                ddt_config_data = {
                    'enable_ddt_auto_backup': system_form.enable_ddt_auto_backup.data,
                    'ddt_backup_frequency_days': system_form.ddt_backup_frequency_days.data,
                    'ddt_backup_retention_days': system_form.ddt_backup_retention_days.data,
                    'ddt_backup_path': system_form.ddt_backup_path.data
                }
                
                fatture_config_data = {
                    'enable_fatture_auto_backup': system_form.enable_fatture_auto_backup.data,
                    'fatture_backup_frequency_days': system_form.fatture_backup_frequency_days.data,
                    'fatture_backup_retention_days': system_form.fatture_backup_retention_days.data,
                    'fatture_backup_path': system_form.fatture_backup_path.data
                }
                
                # Salva nel file di configurazione esterno
                update_system_config(system_config_data)
                update_chat_config(chat_config_data)
                update_clienti_config(clienti_config_data)
                update_ddt_config(ddt_config_data)
                update_fatture_config(fatture_config_data)
                ...
                if system_form.company_logo.data:
                    try:
                        ...
                        # Save path in config - salva sia nel file esterno che nel database
                        logo_path = f"uploads/logos/{filename}"
                        update_company_config({'logo_path': logo_path})
                ...
                flash('Configurazioni sistema aggiornate nel file DBLogix.exe.config!', 'success')
    ...
    if 'submit_chat' in request.form:
        try:
            ...
            # Prepara i dati per il file esterno
            chat_config_data = {
                'enable_chat_auto_cleanup': enable_auto_cleanup,
                'chat_cleanup_frequency_days': cleanup_frequency,
                'enable_chat_auto_backup': enable_auto_backup,
                'chat_backup_retention_days': backup_retention,
                'chat_backup_path': chat_backup_path
            }
            
            # Salva nel file esterno
            update_chat_config(chat_config_data)
            
            # Mantieni compatibilità con database
            try:
                ...
                pass  # Ignora errori database, il file esterno è prioritario
            
            flash('Configurazioni chat aggiornate nel file DBLogix.exe.config!', 'success')
    ...
    if 'submit_clienti' in request.form:
        try:
            ...
            # Prepara i dati per il file esterno
            clienti_config_data = {
                'enable_clienti_auto_backup': enable_auto_backup,
                'clienti_backup_frequency_days': backup_frequency,
                'clienti_backup_retention_days': backup_retention,
                'clienti_backup_path': backup_path
            }
            ...
            flash('Configurazioni clienti aggiornate nel file DBLogix.exe.config!', 'success')
    ...
    if 'submit_ddt' in request.form:
        try:
            ...
            # Prepara i dati per il file esterno
            ddt_config_data = {
                'enable_ddt_auto_backup': enable_auto_backup,
                'ddt_backup_frequency_days': backup_frequency,
                'ddt_backup_retention_days': backup_retention,
                'ddt_backup_path': backup_path
            }
            ...
            flash('Configurazioni DDT aggiornate nel file DBLogix.exe.config!', 'success')
    ...
    if 'submit_fatture' in request.form:
        try:
            ...
            # Prepara i dati per il file esterno
            fatture_config_data = {
                'enable_fatture_auto_backup': enable_auto_backup,
                'fatture_backup_frequency_days': backup_frequency,
                'fatture_backup_retention_days': backup_retention,
                'fatture_backup_path': backup_path
            }
            ...
            flash('Configurazioni fatture aggiornate nel file DBLogix.exe.config!', 'success')
...
@admin_bp.route('/configurazioni/reload-config', methods=['POST'])
@admin_required
def reload_external_config():
    """Ricarica la configurazione dal file DBLogix.exe.config"""
    try:
        reload_config()
        flash('Configurazione ricaricata con successo dal file DBLogix.exe.config!', 'success')
    except Exception as e:
        flash(f'Errore nel ricaricare la configurazione: {str(e)}', 'danger')
